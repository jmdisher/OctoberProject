...
public class TestTickRunner
{
	...
	public void shockwaveOneCuboid()
	{
		...
		runner.startNextTick();
		runner.startNextTick();
		TickSnapshot snap1 = runner.startNextTick();
		Assert.assertEquals(6, snap1.cuboids().get(address).scheduledBlockMutations().size());
		TickSnapshot snap2 =runner.waitForPreviousTick();
		Assert.assertEquals(36, snap2.cuboids().get(address).scheduledBlockMutations().size());
		runner.startNextTick();
		TickSnapshot snap3 = runner.startNextTick();
		Assert.assertEquals(0, snap3.cuboids().get(address).scheduledBlockMutations().size());
		runner.shutdown();
		
		// 1 + 6 + 36 = 43.
		Assert.assertEquals(1, snap1.stats().countOfCuboidMutationsRun());
		Assert.assertEquals(6, snap2.stats().countOfCuboidMutationsRun());
		Assert.assertEquals(36, snap3.stats().countOfCuboidMutationsRun());
	}
	...
	public void shockwaveMultiCuboids()
	{
		...
		runner.startNextTick();
		TickSnapshot snap1 = runner.startNextTick();
		Assert.assertEquals(4, snap1.cuboids().values().stream().filter((TickSnapshot.SnapshotCuboid cuboid) -> !cuboid.scheduledBlockMutations().isEmpty()).count());
		TickSnapshot snap2 = runner.startNextTick();
		Assert.assertEquals(7, snap2.cuboids().values().stream().filter((TickSnapshot.SnapshotCuboid cuboid) -> !cuboid.scheduledBlockMutations().isEmpty()).count());
		TickSnapshot snap3 = runner.startNextTick();
		Assert.assertEquals(0, snap3.cuboids().values().stream().filter((TickSnapshot.SnapshotCuboid cuboid) -> !cuboid.scheduledBlockMutations().isEmpty()).count());
		runner.shutdown();
	...
	public void dependentEntityChanges()
	{
		...
		runner.startNextTick();
		TickSnapshot finalSnapshot = runner.waitForPreviousTick();
	...
	{
		TickSnapshot[] snapshotRef = new TickSnapshot[1];
		...
		Map<CuboidAddress, TickSnapshot.SnapshotCuboid> initialCuboids = snapshotRef[0].cuboids();
		...
		Map<CuboidAddress, TickSnapshot.SnapshotCuboid> laterCuboids = snapshotRef[0].cuboids();
	...
	public void saturatingMutations()
	{
		...
		runner.startNextTick();
		runner.startNextTick();
		TickSnapshot snap1 = runner.waitForPreviousTick();
	...
	public void waterFlowOnBlockBreakOnly()
	{
		...
		for (int j = 0; j < ticksToPass; ++j)
		{
			...
			runner.startNextTick();
			snapshot = runner.waitForPreviousTick();
			Assert.assertTrue(snapshot.cuboids().values().stream().filter((TickSnapshot.SnapshotCuboid snapCuboid) -> (null != snapCuboid.blockChanges())).toList().isEmpty());
			Assert.assertEquals(ENV.special.AIR.item().number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, emptyLocation.getBlockAddress()));
		}
		...
		Assert.assertEquals(0, snapshot.cuboids().values().iterator().next().scheduledBlockMutations().size());
		Assert.assertEquals(1, snapshot.cuboids().values().iterator().next().blockChanges().size());
		Assert.assertEquals(WATER_SOURCE.item().number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, emptyLocation.getBlockAddress()));
		Assert.assertEquals((byte)2, snapshot.cuboids().get(address).completed().getData7(AspectRegistry.BLOCK_DEFINED_BYTE, emptyLocation.getBlockAddress()));
		Assert.assertEquals(ENV.special.AIR.item().number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, leafLocation.getBlockAddress()));
	...
	public void breakGrowingWheat()
	{
		...
		runner.waitForPreviousTick();
		long nextCommit = 1L;
		runner.enqueueEntityChange(entityId, _wrapSubAction(entity, new EntitySubActionPlaceSelectedBlockGeneric(location, null)), nextCommit);
		nextCommit += 1L;
		runner.startNextTick();
	...
	public void fallingCreature()
	{
		...
		TickSnapshot snapshot = _passSomeTime(runner, 100L);
	...
	public void futureMutation()
	{
		...
		Assert.assertEquals(1, snap.stats().countOfEntityActionsRun());
	...
	public void flowingLavaLight()
	{
		...
		Assert.assertEquals(lavaSource.item().number(), topCuboid.getData15(AspectRegistry.BLOCK, strong.getBlockAddress()));
		Assert.assertEquals((byte)1, topCuboid.getData7(AspectRegistry.BLOCK_DEFINED_BYTE, strong.getBlockAddress()));
		Assert.assertEquals(ENV.lighting.getLightEmission(lavaSource, false), topCuboid.getData7(AspectRegistry.LIGHT, strong.getBlockAddress()));
		Assert.assertEquals(ENV.lighting.getLightEmission(lavaSource, false) - 1, topCuboid.getData7(AspectRegistry.LIGHT, strong.getRelative(0, 0, 1).getBlockAddress()));
		Assert.assertEquals(lavaSource.item().number(), topCuboid.getData15(AspectRegistry.BLOCK, weak.getBlockAddress()));
		Assert.assertEquals((byte)2, topCuboid.getData7(AspectRegistry.BLOCK_DEFINED_BYTE, weak.getBlockAddress()));
		Assert.assertEquals(ENV.lighting.getLightEmission(lavaSource, false), topCuboid.getData7(AspectRegistry.LIGHT, weak.getBlockAddress()));
		Assert.assertEquals(ENV.lighting.getLightEmission(lavaSource, false) - 1, topCuboid.getData7(AspectRegistry.LIGHT, weak.getRelative(0, 0, 1).getBlockAddress()));
	...
	public void emittersWithDoors()
	{
		...
		runner.enqueueEntityChange(1, _wrapSubAction(entity1, new EntitySubActionPlaceSelectedBlockGeneric(emitterSpace1, FacingDirection.EAST)), 1L);
		runner.enqueueEntityChange(2, _wrapSubAction(entity2, new EntitySubActionPlaceSelectedBlockGeneric(emitterSpace2, FacingDirection.EAST)), 1L);
		runner.enqueueEntityChange(3, _wrapSubAction(entity3, new EntitySubActionPlaceSelectedBlockGeneric(doorSpace1_1, null)), 1L);
		runner.enqueueEntityChange(4, _wrapSubAction(entity4, new EntitySubActionPlaceSelectedBlockGeneric(wireSpace2_1, null)), 1L);
		runner.startNextTick();
		runner.waitForPreviousTick();
		runner.enqueueEntityChange(3, _wrapSubAction(entity3, new EntitySubActionPlaceSelectedBlockGeneric(doorSpace1_2, null)), 2L);
		runner.enqueueEntityChange(4, _wrapSubAction(entity4, new EntitySubActionPlaceSelectedBlockGeneric(wireSpace2_2, null)), 2L);
	...
	public void diodes()
	{
		...
		// Run phase1.
		// These 2 should fail.
		runner.enqueueEntityChange(1, _wrapSubAction(entity1, new EntitySubActionPlaceSelectedBlockGeneric(emitterActiveDirect.getRelative(0, 1, 0), FacingDirection.EAST)), 1L);
		runner.enqueueEntityChange(2, _wrapSubAction(entity2, new EntitySubActionPlaceSelectedBlockGeneric(emitterActiveIndirect.getRelative(0, 2, 0), FacingDirection.EAST)), 1L);
		// These 2 should fail.
		runner.enqueueEntityChange(3, _wrapSubAction(entity3, new EntitySubActionPlaceSelectedBlockGeneric(diodeDirect.getRelative(0, -1, 0), FacingDirection.EAST)), 1L);
		runner.enqueueEntityChange(4, _wrapSubAction(entity4, new EntitySubActionPlaceSelectedBlockGeneric(diodeIndirect.getRelative(0, -2, 0), FacingDirection.EAST)), 1L);
		// These 2 should fail.
		runner.enqueueEntityChange(5, _wrapSubAction(entity5, new EntitySubActionPlaceSelectedBlockGeneric(diodeActiveDirect.getRelative(0, 1, 0), null)), 1L);
		runner.enqueueEntityChange(6, _wrapSubAction(entity6, new EntitySubActionPlaceSelectedBlockGeneric(diodeActiveIndirect.getRelative(0, 2, 0), null)), 1L);
		// This will switch "on".
		...
		// These 2 should pass.
		runner.enqueueEntityChange(1, _wrapSubAction(entity1, new EntitySubActionPlaceSelectedBlockGeneric(emitterActiveDirect.getRelative(1, 0, 0), FacingDirection.EAST)), 2L);
		runner.enqueueEntityChange(2, _wrapSubAction(entity2, new EntitySubActionPlaceSelectedBlockGeneric(emitterActiveIndirect.getRelative(2, 0, 0), FacingDirection.EAST)), 2L);
		// These 2 should pass.
		runner.enqueueEntityChange(3, _wrapSubAction(entity3, new EntitySubActionPlaceSelectedBlockGeneric(diodeDirect.getRelative(-1, 0, 0), FacingDirection.EAST)), 2L);
		runner.enqueueEntityChange(4, _wrapSubAction(entity4, new EntitySubActionPlaceSelectedBlockGeneric(diodeIndirect.getRelative(-2, 0, 0), FacingDirection.EAST)), 2L);
		// These 2 should pass.
		runner.enqueueEntityChange(5, _wrapSubAction(entity5, new EntitySubActionPlaceSelectedBlockGeneric(diodeActiveDirect.getRelative(1, 0, 0), null)), 2L);
		runner.enqueueEntityChange(6, _wrapSubAction(entity6, new EntitySubActionPlaceSelectedBlockGeneric(diodeActiveIndirect.getRelative(2, 0, 0), null)), 2L);
	...
	public void inventorySensor()
	{
		...
		runner.enqueueEntityChange(1, _wrapSubAction(entity1, new EntitySubActionPlaceSelectedBlockGeneric(sensorSpace, FacingDirection.EAST)), 1L);
	...
	public void simpleTopLevel()
	{
		...
		Assert.assertEquals(EntityActionPeriodic.ENERGY_COST_PER_TICK_WALKING, newEntity.ephemeralLocal().energyDeficit());
	...
	public void placeVoidLamp()
	{
		...
		EntityActionSimpleMove<IMutablePlayerEntity> onAction = new EntityActionSimpleMove<>(0.0f
			...
			, (byte)5
			, (byte)6
			, new EntitySubActionPlaceSelectedBlockGeneric(onLamp, null)
		);
		...
		EntityActionSimpleMove<IMutablePlayerEntity> offAction = new EntityActionSimpleMove<>(0.0f
			...
			, (byte)5
			, (byte)6
			, new EntitySubActionPlaceSelectedBlockGeneric(offLamp, null)
		);
	...
	public void observePassive()
	{
		...
		runner.startNextTick();
		TickSnapshot endState = runner.waitForPreviousTick();
	...
	@Test
	public void sandStackOnGateOpenDoor()
	{
		// Show that blocks of sand above a gate block are unchanged by the door being open.
		Item sandItem = ENV.items.getItemById("op.sand");
		Block sandBlock = ENV.blocks.fromItem(sandItem);
		...
		Assert.assertEquals(sandBlock.item().number(), completed.getData15(AspectRegistry.BLOCK, gateLocation.getRelative(0, 0, 2).getBlockAddress()));
		
		runner.shutdown();
	}

	@Test
	public void sandStackOnGateBreakBlock()
	{
		// Show that blocks of sand above a gate block will fall through an open door when their supporting block is broken.
		// Note that this is an odd quirk which may be changed in the future:  An open door will support a gravity block but a falling block will pass through it and potentially break it when it solidifies.
		Item sandItem = ENV.items.getItemById("op.sand");
		Block sandBlock = ENV.blocks.fromItem(sandItem);
		Block gateBlock = ENV.blocks.fromItem(ENV.items.getItemById("op.gate"));
		CuboidAddress address = CuboidAddress.fromInt(0, 0, 0);
		...
		CuboidData cuboid = CuboidGenerator.createFilledCuboid(address, ENV.special.AIR);
		cuboid.setData15(AspectRegistry.BLOCK, gateLocation.getBlockAddress(), gateBlock.item().number());
		cuboid.setData7(AspectRegistry.FLAGS, gateLocation.getBlockAddress(), FlagsAspect.FLAG_ACTIVE);
		cuboid.setData15(AspectRegistry.BLOCK, gateLocation.getRelative(0, 0, 1).getBlockAddress(), sandBlock.item().number());
		cuboid.setData15(AspectRegistry.BLOCK, gateLocation.getRelative(0, 0, 2).getBlockAddress(), sandBlock.item().number());
		...
		// Inject the mutation to break the block above the door to see what happens.
		int toughness = ENV.damage.getToughness(sandBlock);
		int fakeEntityId = 1;
		MutationBlockIncrementalBreak mutation = new MutationBlockIncrementalBreak(gateLocation.getRelative(0, 0, 1), toughness, fakeEntityId);
		ScheduledMutation scheduled = new ScheduledMutation(mutation, 0L);
		SuspendedCuboid<IReadOnlyCuboidData> suspended = new SuspendedCuboid<>(cuboid, HeightMapHelpers.buildHeightMap(cuboid), List.of(), List.of(scheduled), Map.of(), List.of());
		runner.setupChangesForTick(List.of(suspended)
			, null
		...
		PassiveEntity passive1 = snapshot.passives().get(1).completed();
		Assert.assertEquals(PassiveType.FALLING_BLOCK, passive1.type());
		Assert.assertEquals(new EntityLocation(5.0f, 6.0f, 2.0f), passive1.location());
		Assert.assertEquals(new EntityLocation(0.0f, 0.0f, 0.0f), passive1.velocity());
		completed = snapshot.cuboids().get(address).completed();
		...
		for (int i = 0; i < 14; ++i)
		{
			...
			// We expect the passive to disappear the tick before it solidifies and spawn a new one in the tick where it solifies.
			if (12 == i)
			{
				Assert.assertTrue(snapshot.passives().isEmpty());
			}
			else
			{
		...
		// Verify that the gate has been replaced and that it dropped as an item.
		completed = snapshot.cuboids().get(address).completed();
		Assert.assertEquals(sandBlock.item().number(), completed.getData15(AspectRegistry.BLOCK, gateLocation.getBlockAddress()));
		Assert.assertEquals(1, snapshot.passives().size());
		PassiveEntity passive2 = snapshot.passives().get(2).completed();
		Assert.assertEquals(PassiveType.ITEM_SLOT, passive2.type());
		Assert.assertEquals(new EntityLocation(5.0f, 6.0f, 1.0f), passive2.location());
		Assert.assertEquals(new EntityLocation(0.0f, 0.0f, 0.0f), passive2.velocity());
		Assert.assertEquals(ItemSlot.fromStack(new Items(gateBlock.item(), 1)), passive2.extendedData());
	...
	@Test
	public void blockMover()
	{
		// Show that we can push and pull blocks with the block mover and a switch.
		CuboidAddress address = CuboidAddress.fromInt(7, 8, 9);
		CuboidData cuboid = CuboidGenerator.createFilledCuboid(address, ENV.special.AIR);
		...
		AbsoluteLocation wireLocation = moverLocation.getRelative(0, 0, 1);
		AbsoluteLocation switchLocation = wireLocation.getRelative(0, 0, 1);
		AbsoluteLocation stone1Location = moverLocation.getRelative(1, 0, 0);
		AbsoluteLocation stone2Location = stone1Location.getRelative(1, 0, 0);
		AbsoluteLocation outsideLocation = stone2Location.getRelative(1, 0, 0);
		cuboid.setData15(AspectRegistry.BLOCK, moverLocation.getBlockAddress(), blockMover.number());
		cuboid.setData7(AspectRegistry.ORIENTATION, moverLocation.getBlockAddress(), FacingDirection.directionToByte(FacingDirection.EAST));
		cuboid.setData15(AspectRegistry.BLOCK, wireLocation.getBlockAddress(), logicWire.number());
		cuboid.setData15(AspectRegistry.BLOCK, switchLocation.getBlockAddress(), switc.number());
		cuboid.setData15(AspectRegistry.BLOCK, stone1Location.getBlockAddress(), STONE_ITEM.number());
		cuboid.setData15(AspectRegistry.BLOCK, stone2Location.getBlockAddress(), STONE_ITEM.number());
		...
		Assert.assertEquals(STONE_ITEM.number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, stone1Location.getBlockAddress()));
		...
		Assert.assertEquals(LogicAspect.MAX_LEVEL, snapshot.cuboids().get(address).completed().getData7(AspectRegistry.LOGIC, wireLocation.getBlockAddress()));
		Assert.assertEquals(FlagsAspect.FLAG_ACTIVE, snapshot.cuboids().get(address).completed().getData7(AspectRegistry.FLAGS, moverLocation.getBlockAddress()));
		Assert.assertEquals(ENV.special.AIR.item().number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, stone1Location.getBlockAddress()));
		Assert.assertEquals(STONE_ITEM.number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, stone2Location.getBlockAddress()));
		...
		Assert.assertEquals(0x0, snapshot.cuboids().get(address).completed().getData7(AspectRegistry.FLAGS, moverLocation.getBlockAddress()));
		Assert.assertEquals(STONE_ITEM.number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, stone1Location.getBlockAddress()));
		Assert.assertEquals(ENV.special.AIR.item().number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, stone2Location.getBlockAddress()));
		Assert.assertEquals(STONE_ITEM.number(), snapshot.cuboids().get(address).completed().getData15(AspectRegistry.BLOCK, outsideLocation.getBlockAddress()));
	...
	private BlockProxy _getBlockProxy(TickSnapshot snapshot, AbsoluteLocation location)
	{
		CuboidAddress address = location.getCuboidAddress();
		TickSnapshot.SnapshotCuboid snapData = snapshot.cuboids().get(address);
	...
	private TickSnapshot _passSomeTime(TickRunner runner, long minMillisToPass)
	{
