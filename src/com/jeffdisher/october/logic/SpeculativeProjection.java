...
public class SpeculativeProjection
{
	...
	public int applyCommittedMutations(Set<CuboidAddress> cuboidsToUnload, List<Either<IMutation, IEntityChange>> updates, long lastLocalCommitIncluded)
	{
		...
		while (null != reverseWrapper)
		{
			...
			if (regenerateReverse)
			{
				...
				IEntityChange forward = change.applyChangeReversible(nullNewMutationsSink);
			...
			else
			{
				...
				if (null != change)
				{
					didApply = change.applyChange(nullNewMutationsSink);
				}
				else
				{
	...
	private boolean _forwardApplySpeculative(Function<AbsoluteLocation, BlockProxy> oldWorldLoader, Map<CuboidAddress, CuboidData> blockCache, IEntityChange change, long commitLevel)
	{
		...
		IEntityChange reverseChange = change.applyChangeReversible(localImmediateNewMutationsSink);
