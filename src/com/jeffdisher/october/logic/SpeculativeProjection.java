...
public class SpeculativeProjection
{
	 ...
	 * @param mutations The list of mutations to apply, authoritative, in-order.
	 ...
	 */
	public int applyCommittedMutations(Set<CuboidAddress> cuboidsToUnload, IMutation[] mutations, long lastLocalCommitIncluded)
	{
		...
		while (null != reverseWrapper)
		{
			...
			MutableBlockProxy thisBlock = _loadMutableBlock(blockCache, reverseWrapper.mutation);
			
			// If these are the same commit number, only run without generating reverse.
			boolean regenerateReverse = (null == nextReverseWrapper) || (nextReverseWrapper.commitLevel != commitNumber);
			if (regenerateReverse)
			{
				IMutation forward = reverseWrapper.mutation.applyMutationReversible(oldWorldLoader, thisBlock, nullNewMutationsSink);
				// We cannot fail to apply the mutation in reverse (a non-apply should be a NullMutation).
				Assert.assertTrue(null != forward);
				// If this is more recent than what we were told was committed, and is not scheduled for unload, we need to re-apply it later.
				if ((commitNumber > lastLocalCommitIncluded) && !cuboidsToUnload.contains(reverseWrapper.mutation.getAbsoluteLocation().getCuboidAddress()))
				{
					MutationWrapper forwardWrapper = new MutationWrapper(commitNumber, forward);
					forwardSpeculative.push(forwardWrapper);
				}
			}
			else
			{
				boolean didApply = reverseWrapper.mutation.applyMutation(oldWorldLoader, thisBlock, nullNewMutationsSink);
		...
		// 3)  Apply these incoming mutations.
		for (IMutation mutation : mutations)
		{
			...
			boolean didApply = mutation.applyMutation(oldWorldLoader, thisBlock, nullNewMutationsSink);
		...
		while (!forwardSpeculative.isEmpty())
		{
			...
			MutableBlockProxy thisBlock = _loadMutableBlock(blockCache, forwardWrapper.mutation);
			
			_forwardApplySpeculative(oldWorldLoader, blockCache, forwardWrapper.mutation, commitLevel, thisBlock);
		}
		_writeBackBlockCache(changedCuboidAddresses, blockCache);
	...
	/**
	 * Applies the given mutation to the local state as speculative and returns the local commit number associated.
	 * 
	 * @param mutation The mutation to apply.
	 * @return The local commit number for this mutation, 0L if it failed to applied and should be rejected.
	 */
	public long applyLocalMutation(IMutation mutation)
	{
		long commitNumber = _nextLocalCommitNumber;
		_nextLocalCommitNumber += 1;
		Map<CuboidAddress, CuboidData> blockCache = new HashMap<>();
		MutableBlockProxy thisBlock = _loadMutableBlock(blockCache, mutation);
		boolean didApply = _forwardApplySpeculative(_buildOldWorldLoader(), blockCache, mutation, commitNumber, thisBlock);
		if (didApply)
		{
	...
	private boolean _forwardApplySpeculative(Function<AbsoluteLocation, BlockProxy> oldWorldLoader, Map<CuboidAddress, CuboidData> blockCache, IMutation mutation, long commitLevel, MutableBlockProxy thisBlock)
	{
		...
		IMutation reverse = mutation.applyMutationReversible(oldWorldLoader, thisBlock, localImmediateNewMutationsSink);
		if (null != reverse)
		{
			_reverseMutations.push(new MutationWrapper(commitLevel, reverse));
			...
			while (!extraLocalMutations.isEmpty())
			{
				...
				if (null != reverse2)
				{
					...
					_reverseMutations.push(new MutationWrapper(commitLevel, reverse2));
				}
			}
		}
		// Return true if we applied the primary mutation.
		return (null != reverse);
	}
	...
	private static record MutationWrapper(long commitLevel, IMutation mutation) {}
}
