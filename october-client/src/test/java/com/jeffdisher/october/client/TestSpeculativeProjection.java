...
public class TestSpeculativeProjection
{
	...
	public void dependentEntityChanges()
	{
		...
		int speculativeCount = projector.applyChangesForServerTick(2L
				...
				, Collections.emptyList()
				, Collections.emptyList()
				, FakeUpdateFactories.entityUpdate(Map.of(), listener.thisEntityState, _wrap(entity, send))
				, List.of()
				, List.of()
				, Collections.emptyList()
				, Collections.emptyList()
		...
		Assert.assertEquals(1, listener.thisEntityState.inventory().sortedKeys().size());
	...
	public void multiPhaseFullCommit()
	{
		...
		EntitySubActionIncrementalBlockBreak blockBreak = new EntitySubActionIncrementalBlockBreak(changeLocation);
	...
	public void movePartialRejection()
	{
		...
		Assert.assertNotNull(listener.thisEntityState);
		EntityLocation initialLocation = listener.thisEntityState.location();
	...
	public void craftBricksSelection()
	{
		...
		{
			EntitySubActionCraft craft = new EntitySubActionCraft(stoneToStoneBrick);
	...
	public void placeAndUseTable()
	{
		...
		EntitySubActionPushItems push = new EntitySubActionPushItems(location, stoneKey, 2, Inventory.INVENTORY_ASPECT_INVENTORY);
		...
		EntitySubActionCraftInBlock craft = new EntitySubActionCraftInBlock(location, stoneToStoneBrick);
		...
		craft = new EntitySubActionCraftInBlock(location, null);
		...
		{
			Assert.assertEquals(craftingTable, proxy.getBlock());
			EntitySubActionIncrementalBlockBreak breaking = new EntitySubActionIncrementalBlockBreak(location);
	...
	public void placeAndLoadFurnace()
	{
		...
		EntitySubActionPushItems pushFail = new EntitySubActionPushItems(location, stoneKey, 1, Inventory.INVENTORY_ASPECT_FUEL);
		...
		EntitySubActionPushItems push = new EntitySubActionPushItems(location, stoneKey, 1, Inventory.INVENTORY_ASPECT_INVENTORY);
		long commit2 = _wrapAndApply(projector, entity, currentTimeMillis, push);
		Assert.assertEquals(2L, commit2);
		
		// Verify that we can store the planks in the fuel inventory.
		EntitySubActionPushItems pushFuel = new EntitySubActionPushItems(location, plankKey, 1, Inventory.INVENTORY_ASPECT_FUEL);
	...
	public void moveWhileStarving()
	{
		...
		Assert.assertNotNull(listener.thisEntityState);
		Entity initialEntity = listener.thisEntityState;
		Assert.assertEquals(OrientationHelpers.YAW_NORTH, listener.thisEntityState.yaw());
		...
		MutableEntity authoritativeMutable = MutableEntity.existing(initialEntity);
		...
		int speculativeCount = projector.applyChangesForServerTick(2L
				...
				, Collections.emptyList()
				, Collections.emptyList()
				, EntityUpdatePerField.update(initialEntity, authoritativeMutable.freeze())
				, List.of()
				, List.of()
				, Collections.emptyList()
				, Collections.emptyList()
	...
	public void orientAndAccelerate()
	{
		...
		int speculativeCount = projector.applyChangesForServerTick(2L
				...
				, Collections.emptyList()
				, Collections.emptyList()
				, FakeUpdateFactories.entityUpdate(Map.of(airAddress, airCuboid), initialEntity, move)
				, List.of()
				, List.of()
				, Collections.emptyList()
				, Collections.emptyList()
	...
	public void attackEvents()
	{
		...
		EntitySubActionAttackEntity attack = new EntitySubActionAttackEntity(creatureId);
		...
		projector.applyChangesForServerTick(2L
				...
				, List.of(new PartialEntityUpdate(PartialEntity.fromCreature(orc)))
				, List.of()
				, Collections.emptyList()
				, Collections.emptyList()
				, Collections.emptyList()
	...
	public void breakWithIncrementalCommit()
	{
		...
		for (int i = 0; i < hitsToBreak; ++i)
		{
			...
			currentTimeMillis += 100L;
			EntitySubActionIncrementalBlockBreak blockBreak = new EntitySubActionIncrementalBlockBreak(dirtLocation);
			long commit1 = _wrapAndApply(projector, entity, currentTimeMillis, blockBreak);
	...
	public void breakBlockNearLight()
	{
		...
		for (int i = 0; i < hitsToBreak; ++i)
		{
			EntitySubActionIncrementalBlockBreak blockBreak = new EntitySubActionIncrementalBlockBreak(targetLocation);
	...
	public void perfLightChanges()
	{
		...
		for (int i = 0; i < iterationCount; ++i)
		{
			...
			currentTimeMillis += 100L;
			EntitySubActionIncrementalBlockBreak breakBlock = new EntitySubActionIncrementalBlockBreak(lanternLocation);
	...
	public void passiveCallbacks() throws Throwable
	{
		...
		PassiveUpdate update = new PassiveUpdate(startPassive.id(), new EntityLocation(1.2f, -4.5f, 7.8f), new EntityLocation(0.0f, 0.0f, -1.0f));
	...
	public void calfGrowUp()
	{
		...
		projector.applyChangesForServerTick(2L
				...
				, List.of(new PartialEntityUpdate(PartialEntity.fromCreature(adult)))
				, List.of()
				, Collections.emptyList()
				, Collections.emptyList()
				, Collections.emptyList()
	...
	public void perf_changesFromServer()
	{
		...
		PassiveUpdate partialPassiveUpdate0 = new PassiveUpdate(passive0.id(), passive0.location(), passive0.velocity());
		PassiveUpdate partialPassiveUpdate1 = new PassiveUpdate(passive1.id(), passive1.location(), passive1.velocity());
		CuboidData cuboid = CuboidGenerator.createFilledCuboid(CuboidAddress.fromInt(0, 0, 0), ENV.special.AIR);
	...
	@Test
	public void repairBlockEvent()
	{
		// Show that the expected event is generated by a repair block action.
		CountingListener listener = new CountingListener();
		int entityId = 1;
		SpeculativeProjection projector = new SpeculativeProjection(entityId, listener, MILLIS_PER_TICK);
		MutableEntity mutable = MutableEntity.createForTest(entityId);
		...
		CuboidAddress address = CuboidAddress.fromInt(0, 0, 0);
		CuboidData cuboid = CuboidGenerator.createFilledCuboid(address, ENV.special.AIR);
		AbsoluteLocation targetBlock = localEntity.location().getBlockLocation();
		BlockAddress standingBlock = targetBlock.getBlockAddress();
		cuboid.setData15(AspectRegistry.BLOCK, standingBlock, STONE_BRICK_ITEM.number());
		cuboid.setDataSpecial(AspectRegistry.DAMAGE, standingBlock, 10);
		int remaining = projector.applyChangesForServerTick(1L
			, List.of()
			, List.of()
			, List.of(cuboid)
		...
		// Repair the block we are standing on and check the event.
		EntityActionSimpleMove<IMutablePlayerEntity> stand = new EntityActionSimpleMove<>(0.0f
			, 0.0f
			, EntityActionSimpleMove.Intensity.STANDING
			, (byte)0
			, (byte)0
			, new EntitySubActionIncrementalBlockRepair(targetBlock)
		);
		currentTimeMillis += MILLIS_PER_TICK;
		long commit = projector.applyLocalChange(stand, currentTimeMillis);
		Assert.assertEquals(1L, commit);
		Assert.assertEquals(new EntityLocation(5.0f, 5.0f, 5.0f), listener.thisEntityState.location());
		Assert.assertEquals(1, listener.events.size());
		Assert.assertEquals(targetBlock, listener.events.get(0).location());
		Assert.assertEquals(entityId, listener.events.get(0).entitySource());
	}
