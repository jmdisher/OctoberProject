 ...
 * that instead.
 */
public class EntitySubActionIncrementalBlockBreak implements IEntitySubAction<IMutablePlayerEntity>
{
	public static final EntitySubActionType TYPE = EntitySubActionType.INCREMENTAL_BREAK_BLOCK;

	public static EntitySubActionIncrementalBlockBreak deserializeFromContext(DeserializationContext context)
	{
		ByteBuffer buffer = context.buffer();
		AbsoluteLocation target = CodecHelpers.readAbsoluteLocation(buffer);
		buffer.getShort();
		return new EntitySubActionIncrementalBlockBreak(target);
	}


	private final AbsoluteLocation _targetBlock;

	public EntitySubActionIncrementalBlockBreak(AbsoluteLocation targetBlock)
	{
