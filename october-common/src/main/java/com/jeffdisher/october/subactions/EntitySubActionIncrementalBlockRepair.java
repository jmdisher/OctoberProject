 ...
 * that instead.
 */
public class EntitySubActionIncrementalBlockRepair implements IEntitySubAction<IMutablePlayerEntity>
{
	public static final EntitySubActionType TYPE = EntitySubActionType.INCREMENTAL_REPAIR_BLOCK;

	public static EntitySubActionIncrementalBlockRepair deserializeFromContext(DeserializationContext context)
	{
		ByteBuffer buffer = context.buffer();
		AbsoluteLocation target = CodecHelpers.readAbsoluteLocation(buffer);
		buffer.getShort();
		return new EntitySubActionIncrementalBlockRepair(target);
	}


	private final AbsoluteLocation _targetBlock;

	public EntitySubActionIncrementalBlockRepair(AbsoluteLocation targetBlock)
	{
	...
	public boolean applyChange(TickProcessingContext context, IMutablePlayerEntity newEntity)
	{
		...
		if (canRepairSomething)
		{
			...
			// Repairing a block expends the same per-tick cost as breaking, no matter the material.
