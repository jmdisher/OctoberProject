...
public class MutationBlockOverwriteByEntity implements IMutationBlock
{
	...
	{
		boolean didApply = CommonBlockMutationHelpers.overwriteBlockIfReplaceableWithFollowUps(context, newBlock, _location, _blockType, _outputDirection, (byte)0, false);
		if (didApply)
		{
			Environment env = Environment.getShared();
