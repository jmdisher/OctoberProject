...
public class TestCreatureMovementHelpers
{
	...
	public void noMoveCentre()
	{
		...
		List<IMutationEntity<IMutableCreatureEntity>> list = CreatureMovementHelpers.prepareForMove(creature, location.getBlockLocation(), false);
		Assert.assertEquals(0, list.size());
		
		location = new EntityLocation(-1.0f, -1.0f, 1.0f);
		creature = _createCow(location);
		list = CreatureMovementHelpers.prepareForMove(creature, location.getBlockLocation(), false);
		Assert.assertEquals(0, list.size());
	}
	...
	public void centreFromBetweenBlocks()
	{
		...
		List<IMutationEntity<IMutableCreatureEntity>> list = CreatureMovementHelpers.prepareForMove(creature, directionHint, false);
		// 3 WEST
		Assert.assertEquals(3, list.size());
		
		// EAST
		directionHint = new AbsoluteLocation(2, 1, 1);
		list = CreatureMovementHelpers.prepareForMove(creature, directionHint, false);
		// 3 SOUTH
		Assert.assertEquals(3, list.size());
