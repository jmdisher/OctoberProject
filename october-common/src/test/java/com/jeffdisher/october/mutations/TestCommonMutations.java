...
public class TestCommonMutations
{
	...
	public void overwriteBoundaryCheckSupport()
	{
		...
		_Events events = new _Events();
		TickProcessingContext context = ContextBuilder.build()
				.lookups(ContextBuilder.buildFetcher((AbsoluteLocation location) -> cuboid.getCuboidAddress().equals(location.getCuboidAddress()) ? BlockProxy.load(location.getBlockAddress(), cuboid) : null), null, null)
				.sinks(new TickProcessingContext.IMutationSink() {
						@Override
						public boolean next(IMutationBlock mutation)
						{
	...
	public void replaceSerialization()
	{
		...
		buffer.flip();
		MutationBlockReplace test = MutationBlockReplace.deserialize(DeserializationContext.empty(ENV
			, buffer
		));
